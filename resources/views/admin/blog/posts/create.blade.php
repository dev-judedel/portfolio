...
@section('content')
    ...
    <section class="relative py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="space-y-2 opacity-0 animate-fade-in" style="animation-delay: 0.1s;">
                <h1 class="text-4xl md:text-5xl font-extralight text-white tracking-tight">Add New Post</h1>
                <p class="text-white/40 font-light">Create a new blog post</p>
            </div>
        </div>
    </section>
    ...
    <section class="pb-16 relative">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <form action="{{ route('admin.blog.posts.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 opacity-0 animate-fade-in-up" style="animation-delay: 0.2s;">
                @csrf
                ...
                <div class="bg-white/5 backdrop-blur-sm rounded-lg border border-white/10 p-8 space-y-6">
                    ...
                    <div>
                        ...
                        <select
                            ...
                            <option value="{{ $category->id }}" {{ old('blog_category_id') == $category->id ? 'selected' : '' }} class="bg-gray-800">
                                {{ $category->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <p class="mt-2 text-xs text-white/40 font-light">Upload an image for your blog post</p>
                    ...
                    <div>
                        ...
                        <select
                            ...
                            required>
                            <option value="draft" {{ old('status') == 'draft' ? 'selected' : '' }} class="bg-gray-800">Draft</option>
                            <option value="published" {{ old('status') == 'published' ? 'selected' : '' }} class="bg-gray-800">Published</option>
                        </select>
                        @error('status')
                        <p class="mt-2 text-sm text-red-400 font-light">{{ $message }}</p>
                        @enderror
