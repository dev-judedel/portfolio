        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between mb-8">
                <div class="space-y-2 opacity-0 animate-fade-in" style="animation-delay: 0.1s;">
                    <h1 class="text-4xl md:text-5xl font-extralight text-white tracking-tight">Projects</h1>
                    <p class="text-white/40 font-light">Manage your portfolio projects and showcases</p>
                </div>

                <div class="opacity-0 animate-fade-in" style="animation-delay: 0.2s;">
                    <a href="{{ route('admin.projects.create') }}" class="inline-flex items-center gap-2 px-6 py-2.5 bg-white/10 hover:bg-white/20 border border-white/20 hover:border-white/30 rounded-lg text-white/80 hover:text-white text-sm font-light uppercase tracking-wider transition-all duration-300">
    ...
    <section class="pb-16 relative">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-white/10">
                                <th class="px-6 py-4 text-left text-xs font-light text-white/60 uppercase tracking-wider">Project</th>
                            ...
                            <tr class="hover:bg-white/5 transition-colors duration-200">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <img src="{{ Storage::url($project->featured_image) }}" alt="{{ $project->title }}" class="w-12 h-12 rounded-lg object-cover bg-white/5">
                                        @else
                                        ...
                                        @endif
                                        <div>
                                            <div class="text-sm font-light text-white/80">{{ $project->title }}</div>
                                            <div class="text-xs text-white/40 font-light">{{ Str::limit($project->description, 50) }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-2 py-1 bg-white/10 rounded text-xs text-white/60 font-light">{{ $project->category ?? '-' }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-white/60 font-light">{{ $project->client ?? '-' }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    @php
                                        $statusColors = [
                                            ...
                                            'planning' => 'bg-yellow-500/10 border-yellow-500/30 text-yellow-400',
                                            'on-hold' => 'bg-red-500/10 border-red-500/30 text-red-400',
                                        ];
                                        $statusColor = $statusColors[$project->status] ?? 'bg-white/10 border-white/20 text-white/60';
                                    @endphp
                                    <span class="inline-flex items-center px-2 py-1 {{ $statusColor }} border rounded text-xs font-light">
                                        {{ ucfirst(str_replace('-', ' ', $project->status ?? 'N/A')) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-sm text-white/60 font-light">{{ $project->order ?? '-' }}</span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('admin.projects.edit', $project) }}" class="p-2 hover:bg-white/10 rounded transition-colors" title="Edit">
            ...
            <div class="text-center py-16 opacity-0 animate-fade-in" style="animation-delay: 0.3s;">
                ...
                <h3 class="text-xl font-light text-white/60 mb-2">No projects yet</h3>
                <p class="text-white/40 text-sm font-light mb-6">Start by adding your first project</p>
                <a href="{{ route('admin.projects.create') }}" class="inline-flex items-center gap-2 px-6 py-2.5 bg-white/10 hover:bg-white/20 border border-white/20 hover:border-white/30 rounded-lg text-white/80 hover:text-white text-sm font-light uppercase tracking-wider transition-all duration-300">
                    <i class="fas fa-plus"></i>
                    <span>Add Project</span>
                </a>
