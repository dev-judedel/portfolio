...
@section('content')
    ...
    <section class="relative py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="space-y-2 opacity-0 animate-fade-in" style="animation-delay: 0.1s;">
                <h1 class="text-4xl md:text-5xl font-extralight text-white tracking-tight">Edit Skill</h1>
                <p class="text-white/40 font-light">Update skill information</p>
            </div>
        </div>
    </section>
    ...
    <section class="pb-16 relative">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <form action="{{ route('admin.skills.update', $skill) }}" method="POST" class="space-y-6 opacity-0 animate-fade-in-up" style="animation-delay: 0.2s;">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-4">
                            <input type="range"
                                   ...
                                   required>
                            <span id="proficiency-value" class="text-white/80 font-light min-w-[50px] text-right">{{ old('proficiency', $skill->proficiency) }}%</span>
                        </div>
                        @error('proficiency')
                        <p class="mt-2 text-sm text-red-400 font-light">{{ $message }}</p>
                        @enderror
